...
pub fn parse_memory_to_kib(memory: &str) -> Result<u64, LsbxError> {
    ...
    let (digits, multiplier_kib): (&str, u64) = if let Some(stripped) = trimmed
        .strip_suffix("GiB")
        .or_else(|| trimmed.strip_suffix("gib"))
    {
        (stripped, 1024 * 1024)
...
mod tests {
    ...
    fn domain_xml_includes_seed_iso_cdrom_when_provided() {
        let params = DomainXmlParams {
            ...
            memory: "1G",
            disk_path: std::path::Path::new("/var/lib/lsbx/vms/lsbx-test-vm.qcow2"),
            seed_iso: Some(std::path::Path::new(
                "/var/lib/lsbx/vms/lsbx-test-vm-cidata.iso",
            )),
        };
