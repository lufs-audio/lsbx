...
- **Two more broker blockers found and fixed during the lifecycle smoke**:
  1. **Empty registry (golden `ci.qcow2` missing)**: `resolve_images_path` honored `--images` and `LSBX_IMAGES_PATH` but not the long-standing `LSBX_IMAGES` host convention that `ci-broker.env` sets, so the broker loaded an empty `ImageRegistry`, failed to resolve profile `ci` → golden `ci-runner`, and used `ci` literally as the golden key (`ci.qcow2` doesn't exist). Fixed `resolve_images_path` to honor `LSBX_IMAGES`. Re-verified: the broker now resolves the `ci` profile and creates the `ci-runner` VM on dispatch.
  2. **gh token keyring-backed**: `gh auth` stores the token in the system keyring, which the non-interactive systemd `lsbx-ci-broker` cannot read — every `gh api` from the service hangges (idle broker, 0 CPU, holding its lock). `gh auth status` shows "Logged in (keyring)". Fixed by adding `GH_TOKEN` to `ci-broker.env` (mode 0600) so `gh` reads it directly. Restarting the broker with `GH_TOKEN` set makes it poll actively again.
- **Broker full dispatch verified** after both fixes: a bounded foreground broker run (same env + `GH_TOKEN`) matched the queued `fail-carnyx` job, dispatched it, and created the `ci-runner` VM (`sbx-18cf8e94...` running) — the complete broker poll → dispatch → create path works. (That smoke VM was destroyed after; the full runner register → job → cleanup lifecycle is the remaining heavyweight item for a maintenance window, per the note above.)
- **Files**: the cutover script and its log are preserved (token-redacted) under `reference/carnyx-cutover/` in this repo.
